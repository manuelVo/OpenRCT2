...
static int peep_update_patrolling_find_bin(rct_peep* peep){
	...
	uint8 bin_positions = footpath_clear_provisional_edges(peep->next_x, peep->next_y, peep->next_z, map_element, map_element->properties.path.edges) & 0xF;
...
static int peep_interact_with_path(rct_peep* peep, sint16 x, sint16 y, rct_map_element* map_element){
	...
	if (footpath_element_has_path_scenery(map_element) &&
		(map_element->flags & MAP_ELEMENT_FLAG_BROKEN) &&
		(footpath_clear_provisional_edges(x, y, map_element->base_height, map_element, map_element->properties.path.edges) & 0xF) != 0xF){
		vandalism_present = 1;
	}
